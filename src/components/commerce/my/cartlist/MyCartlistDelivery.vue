		...
		<div class="sm:col-span-2 p-6 lg:p-8">
			<div class="">
				<div class="flex-1 rounded-xl p-4 sm:p-7 border-r border-gray-200">
					<!-- Section -->
					<div class="py-6 first:pt-0 last:pb-0 first:border-transparent border-gray-200">
						<label
				...
				<div class="flex-1 rounded-xl p-4 sm:p-7">
					...
					>
						...
						<div class="mt-2 space-y-3">
							<template v-if="deliveryAddrListCount > 0">
								<select
									@change="setDeliveryAddress($event.target.value)"
									class="py-2 px-3 pr-11 block w-full border-gray-200 shadow-sm text-base rounded-lg focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400"
									placeholder="최근 배송지 목록"
								>
									<option selected>최근 배송지 목록</option>
									<option
										v-for="(address, addrIndex) in deliveryAddressList.value"
										:key="addrIndex"
							...
							<div class="flex items-center mb-4">
								<input
									v-model="isSaveDeiliveryAddress"
									type="checkbox"
						...
						<div class="grid gap-4 mt-4 lg:mt-6">
							...
							<ul role="list" class="space-y-4 dark:text-white">
								...
								>
									...
									<span
										class="text-base leading-tight text-gray-500 dark:text-gray-400"
										>{{ cItem?.productName }}
...
<script setup lang="ts">
	...
	let isSaveDeiliveryAddress = ref(false);
	...
	const ownerAddressInfoFixed = ref({
		...
		address: "서울특별시",
		zonecode: "99999",
		address_detail: "귀여운 강아지 귀여운 고양이",
	});
	...
	async function updateNextStep() {
		...
		// 배송지 정보 저장
		if (isSaveDeiliveryAddress.value) {
			await MemberService.updatedeliveryinfo(recipientInfo);
		}
	...
	let deliveryAddressList = reactive([]);
	let deliveryAddrListCount = ref(0);
	// 배송지 리스트 가져오기
	function getDeliveryAddressList() {
		MemberService.getdeliveryinfo().then((response) => {
			deliveryAddressList.value = response.data.data?.list;
			deliveryAddrListCount.value = response.data.data?.list.length;
		});
	}

	// 셀렉트박스에서 선택한 주소로 배송지 세팅
	function setDeliveryAddress(arrItem) {
		const { name, address, tel, zonecode, address_detail } = JSON.parse(arrItem);

		recipientInfo.name = name;
		recipientInfo.address = address;
		recipientInfo.tel = tel;
		recipientInfo.zonecode = zonecode;
		recipientInfo.address_detail = address_detail;
	}
