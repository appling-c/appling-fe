...
<script setup lang="ts">
	...
	import { reactive, defineEmits, onMounted, ref, toRaw, computed } from "vue";
	...
	async function getRecentOrderListMember() {
		...
		await OrderService.getRecentOrderListMember(reqestStr).then((response) => {
			...
			orderTotalCount.value = response.data.data.total_elements;
			console.log(orderTotalCount, orderTotalCount.value > 0);
		});
	}
	...
	async function cancelProductOrder() {
		await OrderService.cancelMellerOrder({ order_id: cancelOrderId }).then((response) => {
			modal.hide();
			cancelOrderId = 0;
			return getRecentOrderListMember();
		});
	}
