				...
				<div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
					...
					<div class="p-4 md:p-5">
						...
						<ul class="my-4 space-y-3">
							<li
								v-for="cancelItem in cancelObject?.order_item_list"
								:key="cancelItem.order_item_id"
							>
								<a
...
<script setup lang="ts">
	...
	const $targetEl = document.querySelectorAll("cryptoModal");

	console.log($targetEl);

	let cancelObject = reactive({});
	...
	function orderCancel(orderID) {
		cancelObject = orders.value.find((order) => order.order_id == orderID);
	}
